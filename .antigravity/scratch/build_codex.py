...
def process_file(src_path, dst_path, is_dot=True):
    ...
    content = content.replace("GEMINI.md", "CODEX.md")
    content = content.replace("cloud_antigravity", "openai_codex")
    content = content.replace("Antigravity Control Plane", "Codex Control Plane")
    content = content.replace("Antigravity", "Codex")
    content = content.replace('"agent_name": "antigravity"', '"agent_name": "codex"')
    content = content.replace(
        '"gemini-3.6-flash",\n      "gemini-3.6-pro",\n      "gemini-3.0-flash",\n      "gemini-3.0-pro"',
        '"gpt-5.6-turbo",\n      "gpt-5.6",\n      "gpt-5.5-turbo",\n      "gpt-5.5",\n      "gpt-4o",\n      "o3-mini",\n      "o1"'
    )
    content = content.replace("gemini-3.6-flash", "gpt-5.6-turbo")
...
if os.path.exists(gemini_md_path):
    ...
    c_content = g_content.replace("GEMINI.md", "CODEX.md")
    c_content = c_content.replace(".antigravity", ".codex")
    c_content = c_content.replace("cloud_antigravity", "openai_codex")
    c_content = c_content.replace("Antigravity Control Plane", "Codex Control Plane")
    c_content = c_content.replace("Antigravity", "Codex")
