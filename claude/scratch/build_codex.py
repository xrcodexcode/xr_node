...
antigravity_dir = os.path.join(vault_root, "claude")
...
def process_file(src_path, dst_path, is_dot=True):
    ...
    content = content.replace("claude", target_folder_name)
    content = content.replace("CLAUDE.md", "CODEX.md")
    content = content.replace("anthropic_claude", "openai_codex")
    content = content.replace("Claude Control Plane", "Codex Control Plane")
    content = content.replace("Claude", "Codex")
    content = content.replace('"agent_name": "claude"', '"agent_name": "codex"')
    content = content.replace(
        '"claude-3-7-sonnet",\n      "gemini-3.6-pro",\n      "gemini-3.0-flash",\n      "gemini-3.0-pro"',
        '"gpt-5.6-turbo",\n      "gpt-5.6",\n      "gpt-5.5-turbo",\n      "gpt-5.5",\n      "gpt-4o",\n      "o3-mini",\n      "o1"'
    )
    content = content.replace("claude-3-7-sonnet", "gpt-5.6-turbo")
...
gemini_md_path = os.path.join(vault_root, "CLAUDE.md")
...
if os.path.exists(gemini_md_path):
    ...
    c_content = g_content.replace("CLAUDE.md", "CODEX.md")
    c_content = c_content.replace("claude", ".codex")
    c_content = c_content.replace("anthropic_claude", "openai_codex")
    c_content = c_content.replace("Claude Control Plane", "Codex Control Plane")
    c_content = c_content.replace("Claude", "Codex")
