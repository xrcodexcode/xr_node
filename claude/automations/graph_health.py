...
def main():
    ...
    automations_dir = os.path.join(vault_root, "claude", "automations")
    ...
    health_log_path = os.path.join(vault_root, "claude", "logs", "moc-health.md")
    broken_links_report_path = os.path.join(vault_root, "claude", "reports", "broken-links.md")
    ...
    # 2. Check for broken links using check_vault.py or direct scan
    ...
    if os.path.exists(nodes_dir):
        for f in os.listdir(nodes_dir):
            if f.endswith('.md'):
                ...
                valid_targets[f"nodes/{name_lower}"] = os.path.join(nodes_dir, f)
    ...
    try:
        os.makedirs(os.path.dirname(broken_links_report_path), exist_ok=True)
    ...
    orphan_count = 0
    orphan_path = os.path.join(vault_root, "claude", "reports", "_orphans.md")
    ...
    invalid_tag_count = 0
    invalid_tags_path = os.path.join(vault_root, "claude", "reports", "invalid-tags.md")
    ...
    duplicate_count = 0
    duplicate_path = os.path.join(vault_root, "claude", "reports", "duplicate-candidates.md")
