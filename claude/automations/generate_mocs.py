...
def generate_hierarchy_index(vault_root, mocs_dir):
    ...
    lines += [
        '\n## Gaps\n\n',
        '_Domains without a domain-level MOC yet. Create one using `claude/templates/domain-moc.md`._\n',
        '\n---\n',
...
def generate_home_base(vault_root, mocs_dir):
    ...
    home_lines = [
        ...
        "---\n",
        "*Generated automatically by Claude PKM engine.*\n"
    ]
    try:
...
def main():
    ...
    moc_soft_limit = 50
    moc_hard_limit = 100
    scalability_path = os.path.join(vault_root, "claude", "modules", "graph", "scalability.md")
    ...
    log_file = os.path.join(vault_root, "claude", "logs", "moc-health.md")
