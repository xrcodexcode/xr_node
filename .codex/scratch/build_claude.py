...
def process_file(src_path, dst_path, is_dot=True):
    ...
    content = content.replace("CODEX.md", "CLAUDE.md")
    content = content.replace("openai_codex", "anthropic_claude")
    content = content.replace("Codex Control Plane", "Claude Control Plane")
    content = content.replace("Codex", "Claude")
    content = content.replace('"agent_name": "codex"', '"agent_name": "claude"')
    content = content.replace(
        '"gpt-5.6-turbo",\n      "gemini-3.6-pro",\n      "gemini-3.0-flash",\n      "gemini-3.0-pro"',
        '"claude-3-7-sonnet",\n      "claude-3-5-sonnet",\n      "claude-3-5-haiku",\n      "claude-3-opus"'
    )
    content = content.replace("gpt-5.6-turbo", "claude-3-7-sonnet")
...
if os.path.exists(gemini_md_path):
    ...
    c_content = g_content.replace("CODEX.md", "CLAUDE.md")
    c_content = c_content.replace(".codex", ".claude")
    c_content = c_content.replace("openai_codex", "anthropic_claude")
    c_content = c_content.replace("Codex Control Plane", "Claude Control Plane")
    c_content = c_content.replace("Codex", "Claude")
