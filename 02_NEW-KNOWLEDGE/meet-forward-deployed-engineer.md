...
version: 1
aliases:
  - "Meet Forward Deployed Engineer! (Hottest Job in Silicon Valley) Detailed Study Notes"
...
```mermaid
flowchart TD
    subgraph STT ["1. Speech-to-Text Layer (27:20)"]
        A1["Deepgram"]
        A2["Soniox"]
    end

    subgraph LLM ["2. Reasoning / LLM Layer (27:20)"]
        B1["OpenAI (GPT-4o)"]
        B2["Google Gemini"]
        B3["Anthropic Claude"]
    end

    subgraph TTS ["3. Text-to-Speech Layer (27:41)"]
        C1["ElevenLabs"]
        C2["Cartesia"]
        C3["MiniMax"]
    end

    UserCall["Caller / Customer (13:58)"] --> STT
    STT --> LLM
    LLM --> TTS
    TTS --> UserCall
    
    subgraph Feedback ["FDE Telemetry & Fine-Tuning Loop (27:41)"]
        FDE["Forward Deployed Engineer (Tyler, 27:41)"]
        ModelTeam["In-House Model Team (27:41)"]
    end

    UserCall -. Insights on Failures .-> FDE
    FDE -. Call Data & Edge Cases .-> ModelTeam
    ModelTeam -. Fine-Tuned Models .-> LLM
```
