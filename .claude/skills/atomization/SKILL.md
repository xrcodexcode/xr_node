...
- Every note must use **Frontmatter Schema Version 4** as defined in `.claude/schemas/frontmatter.md`.
- Required fields with no exceptions:
  ...
  - `type`: One of the approved note types from `.claude/schemas/note-types.md`.
  ...
  - `tags`: Array of controlled discovery tags from `.claude/rules/tagging.md`. Empty array `[]` if no tags apply.

### Law 4: Controlled Tag Discipline
- Use **ONLY** the 12 approved discovery tags defined in `.claude/rules/tagging.md`:
...
4. **MOC update guidance**: Inform the user that the MOC index can be refreshed by running:
   ```powershell
   .\.venv\Scripts\python .claude/automations/generate_mocs.py
   ```
...
| 6 | **Controlled tags only** | Every tag in `tags` array exists in `.claude/rules/tagging.md` | `rules/tagging.md` |
