...
def process_file(src_path, dst_path, is_dot=True):
    ...
    content = content.replace("anthropic_claude", "anthropic_claude")
    content = content.replace("Claude Control Plane", "Claude Control Plane")
    content = content.replace("Claude", "Claude")
    content = content.replace('"agent_name": "claude"', '"agent_name": "claude"')
    content = content.replace(
        '"claude-3-7-sonnet",\n      "gemini-3.6-pro",\n      "gemini-3.0-flash",\n      "gemini-3.0-pro"',
        '"claude-3-7-sonnet",\n      "claude-3-5-sonnet",\n      "claude-3-5-haiku",\n      "claude-3-opus"'
    )
    content = content.replace("claude-3-7-sonnet", "claude-3-7-sonnet")
...
if os.path.exists(gemini_md_path):
    ...
    c_content = g_content.replace("CLAUDE.md", "CLAUDE.md")
    c_content = c_content.replace(".claude", ".claude")
    c_content = c_content.replace("anthropic_claude", "anthropic_claude")
    c_content = c_content.replace("Claude Control Plane", "Claude Control Plane")
    c_content = c_content.replace("Claude", "Claude")
