...
def get_vault_root() -> pathlib.Path:
    ...
    # Path is: .claude/automations/lib/constants.py -> parents[3] is vault root
...
class PathsModule(types.ModuleType):
    ...
    def __getattr__(self, name):
        ...
        if name == "VAULT":
            ...
            return vault / ".claude" / "reports"
        elif name == "LOGS":
            return vault / ".claude" / "logs"
        elif name == "RULES":
            return vault / ".claude" / "rules"
        elif name == "SCHEMAS":
            return vault / ".claude" / "schemas"
