...
- Every note must use **Frontmatter Schema Version 4** as defined in `codex/schemas/frontmatter.md`.
- Required fields with no exceptions:
  ...
  - `type`: One of the approved note types from `codex/schemas/note-types.md`.
  ...
  - `tags`: Array of controlled discovery tags from `codex/rules/tagging.md`. Empty array `[]` if no tags apply.

### Law 4: Controlled Tag Discipline
- Use **ONLY** the 12 approved discovery tags defined in `codex/rules/tagging.md`:
...
4. **MOC update guidance**: Inform the user that the MOC index can be refreshed by running:
   ```powershell
   .\.venv\Scripts\python codex/automations/generate_mocs.py
   ```
...
| 6 | **Controlled tags only** | Every tag in `tags` array exists in `codex/rules/tagging.md` | `rules/tagging.md` |
